...
async function deleteArtifact(name: string) {
  const confirmed = await props.confirmAction({
    title: locale.deleteArtifact,
    message: locale.confirmDeleteArtifact.replace('%s', name),
    confirmLabel: locale.deleteArtifact,
    tone: 'danger',
  });
  if (!confirmed) return;
  const response = await DELETE(buildArtifactLink(name));
  if (!response.ok) {
    props.notify(`Failed to delete artifact ${name}`, 'error');
    return;
  }
  ...
  <div class="ui fluid container">
    <div class="action-view-header">
      ...
      <div class="action-commit-summary">
        ...
        <template v-else-if="run.commit.shortSHA">
          {{ locale.commit }}
        ...
        <template v-else>
          {{ locale.triggeredVia.replace('%s', run.triggerEvent || 'act') }}
        </template>
      ...
      <div class="action-view-left">
        ...
        <div class="flex-items-block action-view-sidebar-list">
          <div
            ...
            :key="item.job.id"
          >
            <a class="job-brief-link silenced" :href="item.job.link">
          ...
          <div v-if="visibleJobListItems.length === 0" class="sidebar-empty-state">
            No jobs captured for this run yet.
          </div>
        </div>
          ...
          <div class="flex-items-block action-view-sidebar-list">
            <div class="item" v-for="artifact in artifacts" :key="artifact.name">
              <template v-if="artifact.status !== 'expired'">
                ...
                <a v-if="props.managementMode && run.canDeleteArtifact" class="silenced" @click="deleteArtifact(artifact.name)">
        ...
        <div class="flex-items-block action-view-sidebar-list">
          <div class="item workflow-file-item" :title="run.workflowID">
            <SvgIcon name="octicon-file-code" class="tw-text-text"/>
            <span class="gt-ellipsis">{{ locale.workflowFile }}</span>
            <span class="workflow-file-path gt-ellipsis">{{ run.workflowID }}</span>
          </div>
        </div>
      </div>
...
.job-brief-link:hover {
  color: inherit;
}

.job-brief-link > .gt-ellipsis {
  flex: 1;
}
