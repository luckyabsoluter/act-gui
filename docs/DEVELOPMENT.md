...
The daemon listens on `localhost:27979` by default. `--act-gui-port <port>` is an act-gui-only argument: the CLI strips it before handing control to act, starts or connects to the daemon on that port, and passes the same port to the internal daemon process.
